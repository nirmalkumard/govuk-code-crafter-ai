...
// Define the interface for the Anthropic API response
...
// List of models by preference order
...
export const generateCodeWithAnthropic = async (
  ...
  let modelToUse = modelOverride || ANTHROPIC_MODELS[0];
  let attempts = 0;
  ...
  while (attempts < (modelOverride ? 1 : ANTHROPIC_MODELS.length)) {
    if (!modelOverride) {
      modelToUse = ANTHROPIC_MODELS[attempts];
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (responseData.error?.message?.includes('not available') ||
            responseData.error?.message?.includes('overloaded')) {
      ...
      const data: AnthropicResponse = responseData;
      
      if (!data.content || !data.content[0]?.text) {
        throw new Error('No generated content received from Anthropic');
      }
